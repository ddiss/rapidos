...
func init() {
	manifest := rapidos.Manifest{
		...
		// The Init package will be executed immediately when the image
		// is booted.
		...
		// Kmods contains a list of kernel modules that should be placed
		// in the initramfs image. modules.dep dependencies will be
		// automatically added.
		Kmods: []string{"zram", "lzo"},
		// Bins specifies which binaries from the host system should be
		// included in the image.
		Bins:  []string{"mkfs.xfs"},
		// Additional miscellaneous files can be listed in Files.
		Files: []string{},
